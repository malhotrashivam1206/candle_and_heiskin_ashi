...
data_file_path = "ohlc_data-MCX-4.csv"
graph_file_path = "candlestick_graph-MCX-4.html"
...
def update_graph(n, interval, chart_type):
    ...
    if chart_type == 'normal':
        fig = go.Figure(data=[go.Candlestick(x=resampled_data.index,
                                             ...
                                             increasing_fillcolor='rgb(0, 255, 0)',
                                             decreasing_fillcolor='rgb(255, 0, 0)')])
    elif chart_type == 'heikin_ashi':
        heikin_ashi_data = resampled_data.copy()

        # Calculate Heikin Ashi values
        heikin_ashi_data['ha_close'] = (heikin_ashi_data['open'] + heikin_ashi_data['high'] +
                                        heikin_ashi_data['low'] + heikin_ashi_data['close']) / 4
        heikin_ashi_data['ha_open'] = (heikin_ashi_data['open'].shift(1) + heikin_ashi_data['close'].shift(1)) / 2
        heikin_ashi_data['ha_high'] = heikin_ashi_data[['high', 'ha_open', 'ha_close']].max(axis=1)
        heikin_ashi_data['ha_low'] = heikin_ashi_data[['low', 'ha_open', 'ha_close']].min(axis=1)

        fig = go.Figure(data=[go.Candlestick(x=heikin_ashi_data.index,
                                             open=heikin_ashi_data['ha_open'],
                                             high=heikin_ashi_data['ha_high'],
                                             low=heikin_ashi_data['ha_low'],
                                             close=heikin_ashi_data['ha_close'],
                                             increasing_line_color='green',
                                             decreasing_line_color='red',
                                             increasing_fillcolor='rgb(0, 255, 0)',
                                             decreasing_fillcolor='rgb(255, 0, 0)')])
    ...
    fig.update_layout(title='Live Candlestick Graph',
                      xaxis_rangeslider_visible=False,
                      yaxis_title='Price',
                      template='plotly_dark')

    # Save the updated graph to an HTML file
